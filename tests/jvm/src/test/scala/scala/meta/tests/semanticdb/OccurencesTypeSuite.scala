...
class OccurencesTypeSuite extends SemanticdbSuite {

	override def customizeConfig(config: SemanticdbConfig): SemanticdbConfig =
		super.customizeConfig(config).copy(occurences = OccurrenceMode.SymbolAndType)
