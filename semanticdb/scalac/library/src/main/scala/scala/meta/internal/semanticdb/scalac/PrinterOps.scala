...
trait PrinterOps { self: SemanticdbOps =>
  ...
  def showSynthetic(tpe: g.Type, actualType: Option[s.Type]): AttributedSynthetic = tpe match {
    ...
    case PolyType(_, _: TypeBounds) =>
      ...
      AttributedSynthetic.empty
    case _ => showSynthetic(g.TypeTree(tpe), actualType)
  }
  def showSynthetic(what: g.Tree, actualType: Option[s.Type]): AttributedSynthetic = {
    val out = new StringWriter()
    val printer = SyntheticCodePrinter(out)
    printer.print(what)
    val occurrences = printer.occurrences.map {
      case ((start, end), symbol) => SyntheticRange(start, end, symbol, actualType)
    }.toList
